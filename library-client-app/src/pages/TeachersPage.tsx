...
import { ReaderTeacherEditForm } from "../components/ReaderTeacherEditForm";
import { FaTrash, FaEdit, FaPlus } from "react-icons/fa";

export default function TeachersPage() {
  const [filter, setFilter] = useState("");
  const HOST = "http://127.0.0.1:8000/";
  const service = "teachers";
  const { readers, loading, error, refetch } = useTeachers(HOST, service);

  const [editingReader, setEditingReader] = useState<Teacher | null>(null);
  const [creatingNewReader, setCreatingNewReader] = useState(false);
  const [localReaders, setLocalReaders] = useState<Teacher[]>([]);

  useEffect(() => {
    if (readers.length > 0) setLocalReaders(readers);
  }, [readers]);

  const createReader = async (newReader: Teacher) => {
    try {
      const params = new URLSearchParams({
        name: newReader.name,
        birth_date: newReader.birth_date.toISOString().slice(0, 10),
        library_id: newReader.library_id.toString(),
        subject: newReader.subject,
        school_addr: newReader.school_addr,
      });

      const url = `${HOST}readers/teacher?${params.toString()}`;
  ...
  const saveReader = async (updatedReader: Teacher) => {
    try {
      const params = new URLSearchParams({
        name: updatedReader.name,
        birth_date: updatedReader.birth_date.toISOString().slice(0, 10),
        subject: updatedReader.subject,
        school_addr: updatedReader.school_addr,
      });

      const url = `${HOST}readers/teachers/${updatedReader.id}?${params.toString()}`;
  ...
  const filteredReaders = localReaders.filter((r) => {
    ...
    return (
      ...
      r.subject.toLowerCase().includes(filterLower) ||
  ...
  if (editingReader) {
    return (
      <div className="max-w-4xl mx-auto p-6">
        <h1 className="text-2xl font-semibold mb-4">Редактирование учителя</h1>
        <ReaderTeacherEditForm
  ...
  if (creatingNewReader) {
    return (
      <div className="max-w-4xl mx-auto p-6">
        <h1 className="text-2xl font-semibold mb-4">Новый учитель</h1>
        <ReaderTeacherEditForm
          reader={{
            id: 0,
            name: "",
  ...
  return (
    <div className="max-w-5xl mx-auto p-6">
      <h1 className="text-3xl font-bold mb-6 text-center">👩‍🏫 Учителя</h1>

      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        <input
          type="text"
          placeholder="Поиск по имени, предмету, школе..."
          ...
          {filteredReaders.map((reader) => (
            <div key={reader.id} className="p-4 bg-white rounded-lg shadow relative border border-gray-200">
              ...
              <ul className="text-sm text-gray-600 space-y-1">
                ...
                <li><strong>Предмет:</strong> {reader.subject}</li>
                <li><strong>Школа:</strong> {reader.school_addr}</li>
